<template>
  ...
  >
    ...
    <Transition name="expand">
      <div v-if="expanded && activity.log" class="mt-3">
        <pre class="font-mono text-[11px] text-muted bg-bg border border-border rounded-lg p-3 overflow-x-auto max-h-60 overflow-y-auto whitespace-pre-wrap">{{ activity.log }}</pre>
      </div>
    </Transition>
  </div>
</template>
...
const props = defineProps<{ activity: UpsunActivity }>()
